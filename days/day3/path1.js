

module.exports = function path1(){
    // if(partC=="1"){
    //     alert("Rats have moved to Hudson Valley."+
    //     " They make a town call Ratsville."+
    //     " They are happy with their pizza."+
    //     " You mayor get to be mayor another day.")
    //     alert("Congrats Goodbye")
    //   }else if(partC=="2"){
    //     alert("Rats have been given cup noodles. Rats now have higher blood pressue. They die younger and prettier. ")
    //     alert("Those who survive become sluggish but hilarious.")
    //     alert("You get to be mayor for another day")

    //     alert("Rat start comedy open mic. Rats start Rat Jam open mic. They are now on HBO.")
    //     alert("With humorous jabs at the city, one becomes popular as Rat Pryor. Rat Pryor says there is no beef on the pizza, just on your face. ")
    //     alert("You’re asked about the comment at a press conference. “Mayor how do you feel?")
    //     let howDoYouFeel = prompt("'Mayor, how dou you feel?(1) We got bigger things to worry about than name calling like protecting the community or (2) New Yorkers are known for their sense of humor, and that’s what makes this city great.")
    //     if(howDoYouFeel=="1"){
    //       alert("Community Board:The people are complaining about mocking rats. They just sit near the curb and laugh. ")

    //     }else if(howDoYouFeel=="2"){
          
    //     }
    //     alert("Congrats Goodbye")



    //   }else{
    //     alert("Rats kept eating pizza. They start pizza delivery company."+
    //     " The Rats fund your opponent and you in the next election."+
    //     " The Rats go nowhere with pizza delivery, and become the new Dutch of New York and furniture retailer that also sells merch. Good luck ")
    //   }
}